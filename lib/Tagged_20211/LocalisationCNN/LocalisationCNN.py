...
class LocalisationCNN:
    ...
    def __init__(self, initial_learning_rate=1e-3, training_keep_prob=0.8):
        ...
        # Placeholders & Vars
        self.x = tf.compat.v1.placeholder(tf.float32, [None, self.img_size, self.img_size], name="x1")
        self.y = tf.compat.v1.placeholder(tf.float32, [None, self.output_size], name="y")
        ...
        self.loss = tf.keras.losses.MeanSquaredError()(self.y, self.y_)
        ...
        # !!! By default the update ops are placed in tf.compat.v1.GraphKeys.UPDATE_OPS, so they need to be added as a dependency to the train_op
        ...
        var_name_mapping = {
            'layer1/conv/kernel': 'layer1/conv_W',
            'layer1/conv/bias': 'layer1/conv_b',
            
            'layer2/conv/kernel': 'layer2/conv_W',
            'layer2/conv/bias': 'layer2/conv_b',
            
            'layer3/conv/kernel': 'layer3/conv_W',
            'layer3/conv/bias': 'layer3/conv_b',
            
            'layer4/conv/kernel': 'layer4/conv_W',
            'layer4/conv/bias': 'layer4/conv_b',
            
            'layer5/conv/kernel': 'layer5/conv_W',
            'layer5/conv/bias': 'layer5/conv_b',
        }
    ...
    def init_model_dir(self):
        ...
        model_dir = "../model/{}".format(self.unique_model_name)
    ...
    def build_network(self, input_x, output_size, is_training, keep_prob=0.8):
        ...
        with tf.compat.v1.variable_scope("layer1"):
            conv1 = tf.keras.layers.Conv2D(
                filters=32, kernel_size=3, strides=1, padding="valid", activation=None, name="conv"
            )(input_x)
            conv1 = tf.keras.layers.BatchNormalization(name='batch_normalization')(conv1, training=is_training)
            conv1 = tf.nn.relu(conv1)
            pool1 = tf.keras.layers.MaxPooling2D(pool_size=3, strides=2, padding="valid", name="pool")(conv1)

        # Layer 2
        with tf.compat.v1.variable_scope("layer2"):
            conv2 = tf.keras.layers.Conv2D(
                filters=128, kernel_size=3, strides=1, padding="valid", activation=None, name="conv"
            )(pool1)
            conv2 = tf.keras.layers.BatchNormalization(name='batch_normalization')(conv2, training=is_training)
            conv2 = tf.nn.relu(conv2)
            pool2 = tf.keras.layers.MaxPooling2D(pool_size=3, strides=2, padding="valid", name="pool")(conv2)
        ...
        with tf.compat.v1.variable_scope("layer3"):
            ...
            conv3 = tf.keras.layers.BatchNormalization(name='batch_normalization')(conv3, training=is_training)
            conv3 = tf.nn.relu(conv3)
            pool3 = tf.keras.layers.MaxPooling2D(pool_size=3, strides=2, padding="valid", name="pool")(conv3)
        ...
        with tf.compat.v1.variable_scope("layer4"):
            ...
            conv4 = tf.keras.layers.BatchNormalization(name='batch_normalization')(conv4, training=is_training)
            conv4 = tf.nn.relu(conv4)
            pool4 = tf.keras.layers.MaxPooling2D(pool_size=3, strides=2, padding="valid", name="pool")(conv4)
        ...
        with tf.compat.v1.variable_scope("layer5"):
            ...
            conv5 = tf.keras.layers.BatchNormalization(name='batch_normalization')(conv5, training=is_training)
            conv5 = tf.nn.relu(conv5)
            pool5 = tf.keras.layers.MaxPooling2D(pool_size=3, strides=2, padding="valid", name="pool")(conv5)
        ...
        with tf.compat.v1.variable_scope("dense1"):
            dense1 = tf.keras.layers.Dense(1024, activation=tf.nn.relu, use_bias=True, name="dense")(flattened)
        ...
        with tf.compat.v1.variable_scope("dense2"):
            ...
            dense2 = tf.keras.layers.Dense(output_size, activation=None, use_bias=True, name="dense")(dense1)
    ...
    def run_epoch(self, epoch_idx, next_element, is_training=False):
            ...
            while True:
                ...
                if is_training:
                    ...
                    with tf.control_dependencies(self.update_ops):
                        self.train_op = self.optimizer.minimize(self.loss, var_list=tf.compat.v1.trainable_variables())
                    _, merged_summ, cost, acc = self.sess.run(
                        [self.train_op, self.merged, self.loss, self.iou], feed_dict=feed
                    )
                else:
        ...
        msg = "\n{} {}\t- Loss (MSE): {:.3f}, IoU: {:.3f}, time elapsed  : {:.2f} seconds".format(
            epoch_idx + 1, epoch_name, avg_cost, avg_acc, end_loop - start_loop
        )
        log.info(msg)
    ...
    def run_prediction_epoch(self, next_element):
        ...
        # Log
        msg = "\n{}\t- Loss (MSE): {:.3f}, IoU: {:.3f}, time elapsed  : {:.2f} seconds".format(
            "Test", avg_cost, avg_acc, end_loop - start_loop
        )
        log.info(msg)
            ...
            if validation_acc > previous_acc:
                ...
                if last_saved_epoch > 0:  # only do this if we ever saved before
                    if (epoch - last_saved_epoch) >= self.early_stop_threshold:
                        msg = "\nEpoch {} - Early stopping, no more validation acc increase after {} epochs".format(
                            epoch + 1, self.early_stop_threshold
                        )
                        log.info(msg)
                        break
        # /END of epoch loop
