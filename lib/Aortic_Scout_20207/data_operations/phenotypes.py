...
class Aorta(object):
    ...
    def process_itk_data(self):
        """
        Processes the itk image by generating the mesh and centerline
        """
        try:
    ...
    def generate_aorta_regions(self):
        self.lower_aorta, self.upper_aorta = extract_lower_upper_aorta(
            self.centerline, point_list=self.centerline_point_arr, analysis_idx=self.analysis_idx
        )
        self.ascending_arch, self.descending_arch = split_aorta_arch(self.upper_aorta, analysis_idx=self.analysis_idx)
        self.descending_aorta = extract_descending_aorta(
            self.centerline, point_list=self.centerline_point_arr, analysis_idx=self.analysis_idx
        )
    ...
    def generate_diameter(self):
        ...
        try:
            ...
            logger.info("Overall diameter calculated")
        except:
            self.subject_arr.extend([-1, -1, -1])

        self.column_arr.extend(
            ["mean_lower_descending_diameter", "min_lower_descending_diameter", "max_lower_descending_diameter"]
        )
        try:
            ...
            logger.info("Lower aorta diameter calculated")
        except:
            self.subject_arr.extend([-1, -1, -1])
        ...
        try:
            ...
            logger.info("Upper aorta diameter calculated")
        except:
            self.subject_arr.extend([-1, -1, -1])

        self.column_arr.extend(["mean_ascending_arch", "min_ascending_arch", "max_ascending_arch"])
        try:
            mean_ascending_arch, min_ascending_arch, max_ascending_arch = extract_diameter(
                self.vtk_mesh, self.ascending_arch, clean=False
            )
            self.subject_arr.extend([mean_ascending_arch, min_ascending_arch, max_ascending_arch])
            logger.info("Ascending arch diameter calculated")
        except:
            self.subject_arr.extend([-1, -1, -1])

        self.column_arr.extend(["mean_descending_arch", "min_descending_arch", "max_descending_arch"])
        try:
            mean_descending_arch, min_descending_arch, max_descending_arch = extract_diameter(
                self.vtk_mesh, self.descending_arch, clean=False
            )
            self.subject_arr.extend([mean_descending_arch, min_descending_arch, max_descending_arch])
            logger.info("Descending arch diameter calculated")
        except:
            self.subject_arr.extend([-1, -1, -1])

        self.column_arr.extend(["mean_descending_aorta", "min_descending_aorta", "max_descending_aorta"])
        try:
            mean_descending_aorta, min_descending_aorta, max_descending_aorta = extract_diameter(
                self.vtk_mesh, self.descending_aorta, clean=False
            )
            self.subject_arr.extend([mean_descending_aorta, min_descending_aorta, max_descending_aorta])
            logger.info("Descending aorta diameter calculated")
        except:
            self.subject_arr.extend([-1, -1, -1])
    ...
    def generate_centerline_length(self):
        ...
        try:
            ...
            logger.info("Overall length calculated")
        except:
        ...
        try:
            ...
            logger.info("Lower aorta length calculated")
        except:
            self.subject_arr.append(-1)
        ...
        try:
            ...
            logger.info("Arch length calculated")
        except:
            self.subject_arr.append(-1)
        ...
        try:
            ...
            logger.info("Ascending arch length calculated")
        except:
            self.subject_arr.append(-1)
        ...
        try:
            ...
            logger.info("Descending arch length calculated")
        except:
            self.subject_arr.append(-1)
        ...
        try:
            ...
            logger.info("Descending aorta length calculated")
        except:
            self.subject_arr.append(-1)
    ...
    def generate_tortuosity(self):
        ...
        try:
            ...
            logger.info("Overall tortuosity calculated")
        except:
        ...
        try:
            ...
            logger.info("Lower aorta tortuosity calculated")
        except:
            self.subject_arr.extend([-1, -1])
        ...
        try:
            ...
            logger.info("Upper aorta tortuosity calculated")
        except:
            self.subject_arr.extend([-1, -1])
        ...
        try:
            ...
            logger.info("Ascending arch tortuosity calculated")
        except:
            self.subject_arr.extend([-1, -1])
        ...
        try:
            ...
            logger.info("Descending arch tortuosity calculated")
        except:
            self.subject_arr.extend([-1, -1])
        ...
        try:
            ...
            logger.info("Descending aorta tortuosity calculated")
        except:
            self.subject_arr.extend([-1, -1])
    ...
    def generate_curvature_torsion(self):
        ...
        try:
            ...
            logger.info("Overall curvature and torsion calculated")
        except:
            self.subject_arr.extend([-1, -1])
        ...
        try:
            ...
            logger.info("Lower aorta curvature and torsion calculated")
        except:
            self.subject_arr.extend([-1, -1])
        ...
        try:
            ...
            logger.info("Upper aorta curvature and torsion calculated")
        except:
            self.subject_arr.extend([-1, -1])
        ...
        try:
            ...
            logger.info("Ascending arch curvature and torsion calculated")
        except:
            self.subject_arr.extend([-1, -1])
        ...
        try:
            ...
            logger.info("Descending arch curvature and torsion calculated")
        except:
            self.subject_arr.extend([-1, -1])
        ...
        try:
            ...
            logger.info("Descending aorta curvature and torsion calculated")
        except:
            self.subject_arr.extend([-1, -1])
    ...
    def generate_arch_height_width(self):
        ...
        try:
            ...
            logger.info("Arch height and width calculated")
        except:
            self.subject_arr.extend([-1, -1])
...
def extract_diameter(vtk_mesh, vtk_centerline, clean=False):
    if clean:
        (
            ...
            _,
            _,
        ) = clean_centerline(vtk_centerline)
    ...
    diameter_arr = []
    for centerline_idx in range(
        centerline_points.GetNumberOfPoints()
    ):  # we changed this value here to not start from 1
...
def volume_to_smooth_mesh(itk_image, pass_band=0.1, iterations=30, dilate_num=1, erode_num=1):
    ...
    # * We now have the mesh for the aorta
...
def vmtk_centerlines(itk_image, erode_num=1):
    """
    Function computes the aortic centerline using an itk image. The process follows:
    (1) convert voxel image to mesh,
...
def clean_centerline(vtk_centerline):
    """
    Cleans the centerline by reversing the list if the descending aorta is at the end.
    """
    ...
    for idx in range(centerline_points.GetNumberOfPoints()):
        ...
        for i in range(len(curr_point)):
            point = curr_point[i]
            # There should be only one such start_idx/analysis_idx
            if point <= 0:
                start_idx = idx
                analysis_idx = i
...
def extract_lower_upper_aorta(vtk_centerline, point_list=None, analysis_idx=None):
    ...
    if point_list is None and analysis_idx is None:
        # note analysis_idx is one of the axis (x, y, z) that is less than 0
        vtk_centerline, point_list, analysis_idx = clean_centerline(vtk_centerline=vtk_centerline)
    # set final point
    end_point = point_list[-1]  # define This is the start point on the ascending aorta
    ...
    switch_to_upper = False
    # We start from descending aorta base
    for idx in range(len(point_list)):
        ...
        if not switch_to_upper:
            lower_aorta.append(curr_point_set)            
        # Add points to lower aorta until the difference is hit
        else:
            upper_aorta.append(curr_point_set)

        # Calculate the new difference between the current point and the descending aorta base for the desired axis
        difference = np.absolute(lower_descending_compare - compare)

        # idx 1 will always give a larger difference and is excluded
        if idx > 1 and not switch_to_upper:
            # * Using the z axis of end point to determine the transition point
            if difference > previous:
...
def flood_fill(itk_image, dilate_num=1, erode_num=1):
    """
    Performs morphological operations on itk image by dilating and eroding the image.
    """
