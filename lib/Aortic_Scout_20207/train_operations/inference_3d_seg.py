...
from networks.slice_unet import *
from train_operations.make_experiment import *
from data_operations.utils import process_excel
from data_operations.image_utils import write_array, generate_DSC
...
def perform_inferencing():
    ...
    if config.generate_volume:
        write_volume(
            prediction_volume_arr, mask_volume_arr, test_pid, config.output_path + config.experiment_name + "/"
        )
    if config.generate_metrics:
...
def calculate_volume(img_arr, spacing):
    assert np.unique(img_arr)[-1] == 1, "[ERROR] volume calculations requires a mask with voxel values of 1: {}".format(
        np.unique(img_arr)
    )
