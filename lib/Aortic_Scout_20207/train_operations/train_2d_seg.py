...
from networks.UNet_Plus_Plus import *
...
def perform_training():
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=config.batch_size, num_workers=3, pin_memory=True
    )
    validation_dataloader = torch.utils.data.DataLoader(
        validation_dataset, batch_size=config.batch_size, num_workers=3, pin_memory=True
    )
