...
def conv2d_bn_relu(x, filters, training, kernel_size=3, strides=1):
    ...
    x_conv = tf.compat.v1.layers.conv2d(x, filters=filters, kernel_size=kernel_size,
                              strides=strides, padding='same', use_bias=False)
    x_bn = tf.compat.v1.layers.batch_normalization(x_conv, training=training)
...
def conv2d_transpose_bn_relu(x, filters, training, kernel_size=3, strides=1):
    ...
    x_conv = tf.compat.v1.layers.conv2d_transpose(x, filters=filters, kernel_size=kernel_size,
                                        strides=strides, padding='same', use_bias=False)
    x_bn = tf.compat.v1.layers.batch_normalization(x_conv, training=training)
    x_relu = tf.nn.relu(x_bn)
    return x_relu
...
def conv3d_bn_relu(x, filters, training, kernel_size=3, strides=1):
    ...
    x_conv = tf.compat.v1.layers.conv3d(x, filters=filters, kernel_size=kernel_size,
                              strides=strides, padding='same', use_bias=False)
    x_bn = tf.compat.v1.layers.batch_normalization(x_conv, training=training)
    x_relu = tf.nn.relu(x_bn)
    return x_relu
...
def conv3d_transpose_bn_relu(x, filters, training, kernel_size=3, strides=1):
    ...
    x_conv = tf.compat.v1.layers.conv3d_transpose(x, filters=filters, kernel_size=kernel_size,
                                        strides=strides, padding='same', use_bias=False)
    x_bn = tf.compat.v1.layers.batch_normalization(x_conv, training=training)
    x_relu = tf.nn.relu(x_bn)
    return x_relu
...
def residual_unit(x, filters, training, strides=1):
    """
        ...
        """
    orig_x = x
    with tf.compat.v1.name_scope('sub1'):
        x = tf.compat.v1.layers.batch_normalization(x, training=training)
        x = tf.nn.relu(x)
        x = tf.compat.v1.layers.conv2d(x, filters=filters, kernel_size=3, strides=strides, padding='same', use_bias=False)
    with tf.compat.v1.name_scope('sub2'):
        x = tf.compat.v1.layers.batch_normalization(x, training=training)
        x = tf.nn.relu(x)
        x = tf.compat.v1.layers.conv2d(x, filters=filters, kernel_size=3, strides=1, padding='same', use_bias=False)
    ...
    with tf.compat.v1.name_scope('add'):
        if orig_x.shape[3] == x.shape[3] and strides == 1:
            ...
            shortcut = tf.compat.v1.layers.conv2d(orig_x, filters=filters, kernel_size=1, strides=strides,
...
def bottleneck_unit(x, filters, training, strides=1):
    ...
    with tf.compat.v1.name_scope('sub1'):
        x = tf.compat.v1.layers.batch_normalization(x, training=training)
        x = tf.nn.relu(x)
        x = tf.compat.v1.layers.conv2d(x, filters=filters / 4, kernel_size=1, strides=strides, padding='same', use_bias=False)
    with tf.compat.v1.name_scope('sub2'):
        x = tf.compat.v1.layers.batch_normalization(x, training=training)
        x = tf.nn.relu(x)
        x = tf.compat.v1.layers.conv2d(x, filters=filters / 4, kernel_size=3, strides=1, padding='same', use_bias=False)
    with tf.compat.v1.name_scope('sub3'):
        x = tf.compat.v1.layers.batch_normalization(x, training=training)
        x = tf.nn.relu(x)
        x = tf.compat.v1.layers.conv2d(x, filters=filters, kernel_size=1, strides=1, padding='same', use_bias=False)
    with tf.compat.v1.name_scope('add'):
        if orig_x.shape[3] == x.shape[3] and strides == 1:
            shortcut = orig_x
        else:
...
def transpose_upsample2d(x, factor, constant=True):
    ...
    x_up = tf.nn.conv2d_transpose(x, filters=filt, output_shape=output_shape,
...
def build_FCN(image, n_class, n_level, n_filter, n_block, training, same_dim=32, fc=64):
    ...
    for l in range(0, n_level):
        with tf.compat.v1.name_scope('conv{0}'.format(l)):
    ...
    with tf.compat.v1.name_scope('same_dim'):
        for l in range(0, n_level):
    ...
    with tf.compat.v1.name_scope('out'):
        ...
        x = conv2d_bn_relu(x, filters=fc, training=training, kernel_size=1)
        logits = tf.compat.v1.layers.conv2d(x, filters=n_class, kernel_size=1, padding='same')
    return logits
