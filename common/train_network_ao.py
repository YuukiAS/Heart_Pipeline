...
def main(argv=None):
    ...
    if FLAGS.model == 'UNet':
        # image_pl: NXYC
        # label_pl: NXY
        image_pl = tf.compat.v1.placeholder(tf.float32, shape=[None, None, None, 1], name='image')
        label_pl = tf.compat.v1.placeholder(tf.int32, shape=[None, None, None], name='label')
    elif FLAGS.model == 'UNet-LSTM' or FLAGS.model == 'Temporal-UNet':
        # image_pl: NTXYC
        # label_pl: NTXY
        # mask_pl:  NTXY
        image_pl = tf.compat.v1.placeholder(tf.float32, shape=[None, None, None, None, 1], name='image')
        label_pl = tf.compat.v1.placeholder(tf.int32, shape=[None, None, None, None], name='label')
    else:
    ...
    # properly.
    training_pl = tf.compat.v1.placeholder(tf.bool, shape=[], name='training')
    ...
    if FLAGS.model == 'UNet' or FLAGS.model == 'Temporal-UNet':
        ...
        if not FLAGS.joint_train:
            ...
            update_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS, 'LSTM/')
            with tf.control_dependencies(update_ops):
                var_list = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, 'LSTM/')
                print(var_list)
    ...
    with tf.compat.v1.Session() as sess:
        ...
        val_writer = tf.compat.v1.summary.FileWriter(os.path.join(summary_dir, 'validation'), graph=sess.graph)
        ...
        if FLAGS.model == 'UNet-LSTM' and not FLAGS.from_scratch:
            ...
            var_list = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, 'UNet/')
            print('Restore pre-trained UNet weights...')
            saver2 = tf.compat.v1.train.Saver(var_list)
