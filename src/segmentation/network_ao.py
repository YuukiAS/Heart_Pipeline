...
def UNet(images, n_class, n_level, n_filter, n_block, training):
    ...
    with tf.compat.v1.variable_scope('UNet'):
        ...
        for l in range(0, n_level):
            with tf.compat.v1.variable_scope('conv{}'.format(l)):
        ...
        # Upsampling path
        l = n_level - 1
        with tf.compat.v1.variable_scope('conv{}_up'.format(l)):
        ...
        with tf.compat.v1.variable_scope('conv_out'):
            ...
            logits = tf.compat.v1.layers.conv2d(net['conv0_up'], filters=n_class, kernel_size=1, padding='same')
    return logits, net
...
def Temporal_UNet(images, n_class, n_level, n_filter, n_block, training):
    """
        ...
        images: NTXYC
        """
    with tf.compat.v1.variable_scope('Temporal_UNet'):
        net = {}
        x = images
        ...
        with tf.compat.v1.variable_scope('conv_out'):
            ...
            # stability reasons.
            # Refer to https://github.com/tensorflow/tensorflow/issues/2462
            logits = tf.compat.v1.layers.conv3d(net['conv0_up'], filters=n_class, kernel_size=1, padding='same')
    return logits, net
...
def focal_loss(labels, logits, n_class, alpha):
    ...
    label_loss = tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(label_1hot), logits=logits)
...
def Temporal_UNet_Model(images, labels, n_class, n_level, n_filter, n_block, n_step,
    ...
    for t in range(n_step):
        ...
        label_loss = tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(label_1hot), logits=logits_fr)
...
def Conv_LSTM(features, lstm_input_shape, n_hidden, n_step, n_class):
    ...
    with tf.compat.v1.variable_scope('LSTM'):
        ...
        for t in range(n_step):
            ...
            logits = tf.compat.v1.layers.conv2d(cell_output, filters=n_class, kernel_size=1, padding='same', name='conv2d')
