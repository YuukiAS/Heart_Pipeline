...
if __name__ == "__main__":
    ...
    if not os.path.exists(lvot_name):
        logger.error(f"{subject}: Processed LVOT MRI file does not exist")
        sys.exit(1)
    ...
    if args.model == "nnUNet":
        ...
        trained_model_path = os.path.join(
            config.model_dir, "LVOT_20212", "Dataset20212_LVOT", "nnUNetTrainerUMambaBot__nnUNetPlans__2d"
        )
    else:
    ...
    obtain_files_segmented(subject, 20212, "LVOT", os.path.join(data_dir, "seg_lvot.nii.gz"))
