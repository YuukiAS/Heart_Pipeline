...
def process_ukb_trufi(image_directory_path, data_output_path, image_orientation='[1, 0, 0, 0, 1, 0]'):
    ...
    temp_data_gen_path = os.path.join(data_output_path, 'temp')
  
    os.makedirs(temp_data_gen_path, exist_ok=True)
    dcm_arr = glob.glob(os.path.join(image_directory_path, '*.dcm'))
    ...
    for i in arr:
        name = splitter(i)
        current_path = os.path.join(temp_data_gen_path, name)
        img = sitk.ReadImage(i)
        sitk.WriteImage(img, current_path)

    # generate new image 
    # nifti_path = os.path.join(data_output_path, splitter(image_directory_path, order=-2))
    nifti_path = os.path.join(data_output_path, 'out.nii')
