...
class convolution(nn.Module):
    def __init__(self, in_channels, out_channels, norm=True, dropout=True, init=False):
        ...
        self.conv = nn.Conv3d(
            in_channels=in_channels, out_channels=out_channels, kernel_size=(3, 3, 3), stride=1, padding=1, bias=True
        )
        if init:
...
class GenericUNet(nn.Module):
    ...
    def __init__(
        ...
        num_classes,
        num_layers,
        num_convolutions_per_layer=2,
        feature_map_pooling=2,
        dropout=0.0,
        deep_supervision=False,
        dropout_localization=False,
        weight_init=True,
        convolutional_pooling=False,
        convolutional_upsampling=False,
        activation_function=nn.ReLU,
