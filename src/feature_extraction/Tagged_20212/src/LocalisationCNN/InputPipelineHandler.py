...
class InputPipelineHandler:
    ...
    def initialize_dataset(self, all_files, indexes, training=False):
        ...
        return tf.compat.v1.data.make_initializable_iterator(ds)

    # --- Wrapper functions for the tf.dataset input pipeline ---
    def training_data_load_wrapper(self, fpath, idx):
        # print('wrapper', idx)
        return tf.numpy_function(
            func=self.load_hd5_img_and_bbox_from_sequence, inp=[fpath, idx], Tout=[tf.float32, tf.float32]
        )

    def augmentation_wrapper(self, img, coords):
        return tf.numpy_function(func=self._rotate_img_and_bbox, inp=[img, coords], Tout=[tf.float32, tf.float32])

    # def _adjust_bbox_wrapper(img, coords):
    #     return tf.numpy_function(func=self._adjust_bounding_box,
