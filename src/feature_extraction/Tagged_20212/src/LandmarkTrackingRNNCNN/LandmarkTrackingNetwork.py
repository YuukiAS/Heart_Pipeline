...
class LandmarkTrackingNetwork:
    ...
    def build_network(self, image_sequences):
        ...
        dense1 = tf.keras.layers.dense(rnn_outputs, 1024, activation=tf.nn.relu, use_bias=True)
        dense1 = tf.keras.layers.dropout(dense1, rate=1 - self.keep_prob)

        # regression layer
        dense2 = tf.keras.layers.dense(dense1, units=self.output_size, activation=None, use_bias=True)
        return dense2
    ...
    def build_cnn_cell(self, image):
        ...
        """
        with tf.compat.v1.variable_scope("cnn_cell", reuse=tf.AUTO_REUSE):
            # Layer 1
            ...
            pool1 = tf.keras.layers.MaxPooling2D(layer1, 2, 2, padding="VALID", name="pool1")
            ...
            pool2 = tf.keras.layers.MaxPooling2D(layer2, 2, 2, padding="VALID", name="pool2")
            ...
            layer3 = tf.keras.layers.MaxPooling2D(layer3, 2, 2, padding="VALID", name="pool3")
            ...
            dense1 = tf.keras.layers.dense(flattened, 1024, activation=leaky_relu, use_bias=True)
