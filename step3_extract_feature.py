...
def generate_scripts(
    pipeline_dir, data_dir, code_dir, modality, useECG, num_subjects_per_file=100, retest_suffix=None, cpu=True
):
        ...
        for file_i in tqdm(range(1, num_files + 1)):  # start from 1
            ...
            with open(os.path.join(code_step3_dir, f"bat{file_i}.pbs"), "w") as file_script:
                ...
                if file_i == 1:
                    if "aortic_scout" in modality:
                        generate_aggregate(
                            file_aggregate, "aortic_structure", "Aortic_Scout_20207", retest_suffix=retest_suffix
                        )

                    if "la" in modality:
                        generate_aggregate(
                            file_aggregate, "atrial_volume", "Long_Axis_20208", retest_suffix=retest_suffix
                        )
                        generate_aggregate(file_aggregate, "strain_la", "Long_Axis_20208", retest_suffix=retest_suffix)

                    if "sa" in modality:
                        generate_aggregate(
                            file_aggregate, "ventricular_volume", "Short_Axis_20209", retest_suffix=retest_suffix
                        )
                        generate_aggregate(
                            file_aggregate, "wall_thickness", "Short_Axis_20209", retest_suffix=retest_suffix
                        )
